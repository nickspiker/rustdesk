...
impl FluorApp for FluorViewer {
    ...
    ///
    /// Auto-hide, not hide. Fully hiding the bar was tried and AppKit's pairing rule makes it
    /// unusable: `HideMenuBar` forces `HideDock`, so the app ends up with no menu bar AND no Dock,
    /// and its own controls become unreachable without Cmd-Tabbing off the session first. Auto-hide
    /// keeps both one pointer-to-edge gesture away. The cost is that the viewer forwards that same
    /// top-edge gesture to the guest, so reaching for the guest's own menu bar can pull ours in.
    fn covers_menu_bar(&self) -> bool {
        true
    }

    /// The same controls on the Dock icon — a second route to them, since the auto-hidden menu bar
    /// competes with the guest for the top edge. Clicks arrive as `FEvent::MenuItem(id)` from either
    /// surface and are handled in `on_event`, so neither is privileged.
    fn dock_menu(&self) -> Vec<MenuItem> {
        self.menu()
    }

    /// The control spec itself, shared by the menu bar and `dock_menu`. Static for now.
    ...
    fn render(&mut self, target: &mut [u32], ctx: &mut Context) {
        ...
        if self.hud {
            ...
            let line4 = "menu bar (top edge) or dock icon: Local/Remote monitor · Fullscreen · HUD";
